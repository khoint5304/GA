...
struct Customer
{
    ...
    double y;
    double demand;
    bool onlyServicedByStaff;
    double serviceTimeByTruck;
    double serviceTimeByDrone;
};

// Structure to store all problem data
struct ProblemData
{
    int number_staff;
    int number_drone;
    int droneLimitationFightTime;
    int customers_count; // Number of actual customers (including depot)
    vector<Customer> customers;
};

// Structure to hold GA results
struct GAResult
{
    vector<int> best_route;           // Best route found
    double best_distance;             // Distance of the best route
    double min_distance_before_reset; // Minimum distance from best_solutions_before_reset
};
...
// Function to read problem data from file
ProblemData readProblemData(const string &problem_name)
{
    ...
    fs::path data_file = ROOT / "D2D Problem" / "random_data" / (problem_name + ".txt");
    ...
    while (getline(infile, line))
    {
        ...
        if (line.find("number_staff") != string::npos)
        {
            sscanf(line.c_str(), "number_staff %d", &data.number_staff);
        }
        else if (line.find("number_drone") != string::npos)
        {
            sscanf(line.c_str(), "number_drone %d", &data.number_drone);
        }
        else if (line.find("droneLimitationFightTime(s)") != string::npos)
        {
            sscanf(line.c_str(), "droneLimitationFightTime(s) %d", &data.droneLimitationFightTime);
        }
        else if (line.find("Customers") != string::npos)
        {
            sscanf(line.c_str(), "Customers %d", &data.customers_count);
            break; // Stop after reading the number of customers
        }
    }

    // **Add Depot as the first customer (Customer 0)**
    Customer depot;
    depot.x = 0.0;
    depot.y = 0.0;
    depot.demand = 0.0;
    depot.onlyServicedByStaff = false; // Assume depot is not only serviced by staff
    depot.serviceTimeByTruck = 0.0;
    depot.serviceTimeByDrone = 0.0;
    data.customers.push_back(depot); // Add depot at position 0

    // Read column headers
    getline(infile, line); // Read the header line (can be skipped or used for validation)
    ...
    while (getline(infile, line))
    {
        if (line.empty())
            continue; // Skip empty lines
        Customer customer;
        // Use stringstream to parse each column
        ...
        // Read demand
        ss >> customer.demand;
        // Read OnlyServicedByStaff flag
        int tempFlag;
        ss >> tempFlag;
        customer.onlyServicedByStaff = (tempFlag == 1);
        // Read ServiceTimeByTruck(s)
        ss >> customer.serviceTimeByTruck;
        // Read ServiceTimeByDrone(s)
        ss >> customer.serviceTimeByDrone;

        data.customers.push_back(customer); // Add customer to the list
    }

    infile.close();

    // Update the total number of customers to include depot
    // Assuming data.customers_count does not include depot
...
double total_distance(const vector<int> &route, const vector<Customer> &customers)
{
    double total = 0.0;
...
unordered_map<int, unordered_set<int>> find_long_distance_pairs(const vector<Customer> &customers)
{
...
double fitness(const vector<int> &route, const vector<Customer> &customers)
{
...
GAResult solveGA(const ProblemData &data, int pop_size = 50, int generations = 1000, double mutation_rate = 0.1)
{
    int num_customers = data.customers_count; // Now includes depot
    const vector<Customer> &customers = data.customers;
...
#pragma omp single
    ...
    cout << "Number of Staff: " << data.number_staff << endl;
    cout << "Number of Drones: " << data.number_drone << endl;
    cout << "Drone Limitation Flight Time (s): " << data.droneLimitationFightTime << endl;
    cout << "Number of Customers (included depot): " << data.customers_count << endl;
    ...
    */
    ...
    GAResult result = solveGA(data, population_size, generations, mutation_rate);

    // Print the results
    cout << "\nBest distance: " << fixed << setprecision(2) << result.min_distance_before_reset << endl;
    cout << "Best route: ";
    for (size_t i = 0; i < result.best_route.size(); ++i)
    {
        cout << result.best_route[i];
        if (i != result.best_route.size() - 1)
            cout << " -> ";
    }
    cout << endl;
    ...
    for (size_t i = 0; i < result.best_route.size() - 1; ++i)
    {
        int from = result.best_route[i];
        int to = result.best_route[i + 1];
